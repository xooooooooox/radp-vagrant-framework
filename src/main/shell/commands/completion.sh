...
_completion_add_vg_dynamic() {
  ...
  awk '
    ...
    /^# Generated by radp-bash-framework$/ {
      ...
      print "        script_dir=\"$(command -v radp-vf 2>/dev/null)\""
      print "        if [[ -n \"$script_dir\" ]]; then"
      print "            script_dir=\"$(dirname \"$(readlink -f \"$script_dir\" 2>/dev/null || echo \"$script_dir\")\")\""
      print "            radp_vf_home=\"$(dirname \"$script_dir\")\""
      print "        fi"
      print "    fi"
      print "    "
    ...
    /^        .vg.)$/ {
      ...
      print "            "
      print "            # Check if completing option value"
      print "            case \"$prev\" in"
      print "                -C|--cluster)"
...
_completion_add_vg_dynamic_zsh() {
  # For zsh, we pass through the base completion for now
  # Full zsh dynamic completion can be added later
  cat
}
