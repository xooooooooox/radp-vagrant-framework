...
_completion_add_vg_dynamic() {
  ...
  awk '
    ...
    /^        .vg.)$/ {
      ...
      print "                        provisions=\"$(_radp_vf_ruby_completion \"$config_dir\" \"$env_override\" \"provisions\")\""
      print "                        COMPREPLY=($(compgen -W \"$provisions\" -- \"$cur\"))"
      print "                        type __ltrim_colon_completions &>/dev/null && __ltrim_colon_completions \"$cur\""
      print "                    fi"
      print "                    return"
      print "                    ;;"
      ...
      print "                        local provisions"
      print "                        provisions=\"$(_radp_vf_ruby_completion \"$config_dir\" \"$env_override\" \"provisions\")\""
      print "                        COMPREPLY=($(compgen -P \"$prefix\" -W \"$provisions\" -- \"$typed\"))"
      print "                        type __ltrim_colon_completions &>/dev/null && __ltrim_colon_completions \"$typed\""
      print "                    fi"
      print "                    return"
      print "                    ;;"
      print "            esac"
...
_completion_add_vg_dynamic_zsh() {
  ...
  awk '
    ...
    /^# Generated by radp-bash-framework$/ {
      ...
      print "}"
      print ""
      print "# Completion function for provision names (used by --provision-with)"
      print "_radp_vf_provisions() {"
      print "    local config_dir env_override provisions"
      print "    config_dir=\"$(_radp_vf_comp_config_dir)\""
      print "    env_override=\"$(_radp_vf_comp_env)\""
      print "    if [[ -n \"$config_dir\" ]]; then"
      print "        provisions=\"$(_radp_vf_ruby_completion \"$config_dir\" \"$env_override\" \"provisions\")\""
      print "        if [[ -n \"$provisions\" ]]; then"
      print "            local -a prov_array"
      print "            prov_array=(${(f)provisions})"
      print "            prov_array=(\${prov_array//:/\\\\:})"
      print "            _describe \"provision\" prov_array"
      print "        fi"
      print "    fi"
      print "}"
      next
    ...
    /^_radp_vf_vg\(\) \{$/ {
      ...
      print "            _radp_vf_machines"
      print "            # Complete provision names for --provision-with"
      print "            if [[ $CURRENT -gt 1 && \"${words[CURRENT-1]}\" == --provision-with ]]; then"
      print "                _radp_vf_provisions"
      print "            elif [[ \"${words[CURRENT]}\" == --provision-with=* ]]; then"
      print "                compset -P \047*=\047"
      print "                _radp_vf_provisions"
      print "            fi"
      print "            ;;"
      print "    esac"
